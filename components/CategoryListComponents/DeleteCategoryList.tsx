import { disableDeleteCategoryComponentShow } from "@/redux/DeleteCategoryComponentShow";
...
const DeleteCategoryList = () => {
  ...
  const { deleteCategoryComponentID } = useSelector(
  ...
  function handleDelete() {
    ...
    axios.delete(
      `https://ts-api-for-ecomm-product.onrender.com/category/${deleteCategoryComponentID}`,
      {
        headers: {
        ...
        if(res.data){
            ...
            setTimeout(() => {
                setLoading(false)
                dispatch(disableDeleteCategoryComponentShow())
            }, 3000);
        }
    }).catch((err)=>{
        // console.log(err)
...
export default DeleteCategoryList;
